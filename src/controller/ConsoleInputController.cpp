...
const std::string ConsoleInputController::changeNameCasing(
		const std::string &nameToTransform) {
	std::string transformedString = nameToTransform;
	transform(transformedString.begin(), transformedString.end(),
			transformedString.begin(), ::tolower);
	transformedString[0] = toupper(transformedString[0]);
	return transformedString;
}

void ConsoleInputController::addGradeOutput(const StudentGrade &grade,
...
bool ConsoleInputController::isGradeWithinLetterValues(
		const StudentGrade &grade, const int letterIndex) {
	return grade.getGrade()
			>= ApplicationSettings::MinimumGradeValues[letterIndex]
			&& grade.getGrade()
					<= ApplicationSettings::MaximumGradeValues[letterIndex];
}

const vector<std::string> ConsoleInputController::getGradesOutput(
		const vector<StudentGrade> &grades, int letterIndex) {
	vector<std::string> outputForGrade;
...
const bool ConsoleInputController::shouldAddNewGradeRow(const int currentColumn,
		const int gradeInputSize) {
	return currentColumn % this->numberOfColumns == 0
			&& currentColumn < gradeInputSize;
}

const std::string ConsoleInputController::displayGradeInformation(
		const vector<std::string> &outputForGrade, const int letterIndex) {
	std::stringstream output;
	output << "Students earning an "
			<< ApplicationSettings::GradeLetters[letterIndex] << ":" << endl;
	unsigned int currentColumn = 1;

	for (string gradeOutput : outputForGrade) {
		output << std::left << setw(this->columnWidth) << gradeOutput;

		if (this->shouldAddNewGradeRow(currentColumn, outputForGrade.size())) {
			output << endl;
		}
		currentColumn++;
	}
...
const std::stringstream ConsoleInputController::displayGradesOutput() {
	...
	for (int letterIndex = 0;
			letterIndex < ApplicationSettings::GradeLettersSize;
			letterIndex++) {
		vector<std::string> outputForGrade = this->getGradesOutput(grades,
				letterIndex);

		if (!outputForGrade.empty()) {
			output
					<< this->displayGradeInformation(outputForGrade,
							letterIndex);
		}
	}
...
const std::string ConsoleInputController::buildHistogramForGrade(
...
const std::string ConsoleInputController::buildHistogram(
		const vector<StudentGrade> &grades) {
	std::stringstream output;
	for (int letterIndex = 0;
			letterIndex < ApplicationSettings::GradeLettersSize;
			letterIndex++) {
...
ConsoleInputController::ConsoleInputController() {
	...
	this->shouldDisplayGradeWithOutput =
			ApplicationSettings::DefaultDisplayGradeWithOutput;
	this->shouldDisplayHistogramWithOuput =
			ApplicationSettings::DefaultDisplayHistogramWithOutput;
	this->shouldSortStudentGrades =
			ApplicationSettings::DefaultSortStudentGradesByGrade;
}
	...
	try {
		...
		while (index < argc) {
			...
			if (arg == "--help") {
				...
				StudentGrade gradeToAdd = StudentGrade(firstName, lastName,
						grade);
